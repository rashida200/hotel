...
WHERE etat = 'completed' OR etat = 'checked_out';
";
...
<head>
    ...
    <style>
        ...
        .sidebar {
            ...
            align-items: center;
            height: 100%; /* Make sidebar height 100% of its parent */
        }
        ...
        .sidebar nav a i {
            color: #63101A; /* Change Font Awesome icon color */
            margin-right: 10px;
        }
        ...
        .table-container {
            ...
            margin-bottom: 20px;
            overflow-x: auto;
            flex-grow: 1; /* Allow the table to expand and fill remaining space */
    ...
    <div class="wrapper">
        <div class="sidebar">
            ...
            <nav>
                ...
                <!-- Links to filter reservations by status -->
                <a href="list.php"><i class="fas fa-list-alt"></i> All Reservations</a>
                <a href="planned.php?etat=En cours"><i class="fas fa-list-alt"></i> Planned Reservations</a>
                <a href="ongoing.php?etat=Terminée"><i class="fas fa-list-alt"></i> Ongoing Reservations</a>
            </nav>
        </div>
        <div class="main-content">
            ...
            <div class="search-container">
                <form method="get" action="">
                    <input type="date" name="date_arrivee" value="<?= htmlspecialchars($date_arrivee) ?>" placeholder="Arrival Date">
                    <input type="date" name="date_depart" value="<?= htmlspecialchars($date_depart) ?>" placeholder="Departure Date">
                    <button type="submit" class="btn btn-primary">Search</button>
                </form>
            </div>
            <div class="table-container">
                <h2>List of Reservations <?= !empty($etat_reservation) ? '(' . htmlspecialchars($etat_reservation) . ')' : '' ?></h2>
                    ...
                    <tbody>
                        <?php foreach ($reservations as $row): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($row['nbr_adultes_enfants']) ?></td>
                                ...
                                <td class="action-icons">
                                    <a href="edit.php?id_reservation=<?= htmlspecialchars($row['id_reservation']) ?>" title="Edit"><i class="fas fa-edit"></i></a>
                                    <a href="delete.php?id_reservation=<?= htmlspecialchars($row['id_reservation']) ?>" title="Delete" onclick="return confirm('Are you sure you want to delete this reservation?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
